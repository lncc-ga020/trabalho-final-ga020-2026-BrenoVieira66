...
- `notebooks/apresentacao_evolucao_pesquisa.ipynb`: notebook principal do trabalho.
- `apresentacao_evolucao_pesquisa.pdf`: versão em PDF do material.
- `resources/figures/`: figuras usadas no notebook.
- `scripts/euler1d/`: scripts auxiliares usados para gerar os resultados do sistema completo.
...
Os principais resultados com as três equações conservativas foram gerados por:
...
Esses scripts usam CoolProp para calcular propriedades termodinâmicas do CO2. Caso o ambiente Pixi usado na correção não tenha CoolProp instalado, instale essa dependência no ambiente Python antes de executar os scripts.
